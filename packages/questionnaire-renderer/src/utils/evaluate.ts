...
function evaluateOperator(
  ...
  switch (operator) {
    ...
    case 'notEmpty':
      return fieldValue !== null && fieldValue !== undefined && fieldValue !== '';
    case 'in': {
      const list: AnswerValue[] = Array.isArray(compareValue) ? compareValue : [compareValue];
      return list.includes(fieldValue as string | number | boolean | null | undefined);
    }
    case 'notIn': {
      const list: AnswerValue[] = Array.isArray(compareValue) ? compareValue : [compareValue];
      return !list.includes(fieldValue as string | number | boolean | null | undefined);
...
function isSimpleCondition(c: EdgeCondition): c is SimpleCondition {
  return 'field' in c && 'operator' in c;
}

function isCompoundCondition(c: EdgeCondition): c is CompoundCondition {
...
export function getNextNodeId(
  ...
  edges: { source: string; target: string; data?: { condition?: EdgeCondition } }[],
  answers: Record<string, AnswerValue>,
): string | undefined {
